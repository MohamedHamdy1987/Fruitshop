...
// قائمة ثابتة بالمنتجات الشائعة (تُستخدم كـ fallback)
...
async function renderInventory() {
  ...
  try {
    ...
    const prods = products.length ? products : DEFAULT_PRODUCTS.map((p,i) => ({...p, id: `default_${i}`}));
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="card-header" style="cursor:pointer;display:flex;justify-content:space-between"
        onclick="toggleNazilForm()">
        <span>➕ إضافة نازل جديد</span>
        <span id="nazil-form-toggle">▼</span>
      </div>
      <div id="nazil-form" style="padding:14px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px">
          <div>
            ...
            <input id="inv-qty" type="number" step="0.01" min="0" placeholder="0"
              style="width:100%;padding:8px;border:1px solid #ddd;border-radius:8px">
          </div>
          <div>
            <label class="lbl">النولون</label>
            <input id="inv-noulon" type="number" step="0.01" min="0" placeholder="0"
              style="width:100%;padding:8px;border:1px solid #ddd;border-radius:8px">
          </div>
          <div>
            <label class="lbl">المشال</label>
            <input id="inv-mashal" type="number" step="0.01" min="0" placeholder="0"
              style="width:100%;padding:8px;border:1px solid #ddd;border-radius:8px">
          </div>
        </div>
    ...
    container.innerHTML = `<div style="color:var(--red);text-align:center;padding:32px">
      ...
      <button onclick="renderInventory()" style="margin-top:12px;padding:8px 20px;background:var(--green);color:#fff;border:none;border-radius:8px;cursor:pointer">
        🔄 إعادة المحاولة
      </button>
    </div>`;
  }
  ...
  return batches.map((b, i) => {
    ...
    <div id="batch-card-${b.id}" style="background:#fff;border:1px solid #e0e0e0;border-radius:10px;margin-bottom:10px;overflow:hidden">
      ...
      <div style="background:#f0faf5;padding:10px 14px;display:flex;justify-content:space-between;align-items:center">
        <div>
          <div style="font-weight:700;font-size:15px;color:#0e6655">
            ${i+1}. ${b.product_name}
          </div>
        ...
        <div style="text-align:left;display:flex;align-items:center;gap:8px">
          <div>
            <div style="font-weight:700;font-size:16px;color:#0e6655">${N(remQty)} ${b.unit}</div>
            <div style="font-size:11px;color:#888">من ${N(origQty)}</div>
          </div>
          <button onclick="openBatchEditNazil('${b.id}')"
          ...
          <button onclick="deleteNazilBatch('${b.id}')"
      ...
      <div style="padding:6px 14px">
        <div style="background:#e0e0e0;border-radius:4px;height:5px">
          <div style="background:${barClr};width:${pct}%;height:5px;border-radius:4px;transition:.3s"></div>
        </div>
      ...
      <div id="batch-edit-nazil-${b.id}" style="display:none;padding:12px 14px;border-top:1px solid #eee;background:#fffde7">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
          <div><label style="font-size:12px;color:#666">الكمية</label>
            <input id="be-qty-${b.id}" type="number" step="0.01" value="${origQty}"
          ...
          <div><label style="font-size:12px;color:#666">المتبقي</label>
            <input id="be-rem-${b.id}" type="number" step="0.01" value="${remQty}"
              style="width:100%;padding:6px;border:1px solid #ddd;border-radius:6px"></div>
          <div><label style="font-size:12px;color:#666">النولون</label>
            <input id="be-noulon-${b.id}" type="number" step="0.01" value="${noulon}"
              style="width:100%;padding:6px;border:1px solid #ddd;border-radius:6px"></div>
          <div><label style="font-size:12px;color:#666">المشال</label>
            <input id="be-mashal-${b.id}" type="number" step="0.01" value="${mashal}"
              style="width:100%;padding:6px;border:1px solid #ddd;border-radius:6px"></div>
        </div>
        <div style="display:flex;gap:8px;margin-top:10px">
          <button onclick="saveNazilBatchEdit('${b.id}')"
      ...
      <div style="padding:8px 14px">
        <button onclick="goToProduct('${b.id}')"
          style="width:100%;background:var(--green);color:#fff;border:none;border-radius:8px;padding:9px;cursor:pointer;font-weight:700">
          🛒 بيع من هذه الدفعة
        </button>
      </div>
    </div>`;
...
async function addInventoryBatch() {
  ...
  try {
    ...
    await renderInventory(); // إعادة رسم كاملة من DB

  } catch(e) { AppError.log('addInventoryBatch', e, true); }
  finally { btn.disabled = false; btn.textContent = '➕ إضافة نازل'; }
}
...
async function saveNazilBatchEdit(batchId) {
  ...
  try {
    await sb.from('incoming_batches')
      .update({ quantity: qty, remaining_qty: rem, noulon, mashal })
      .eq('id', batchId)
