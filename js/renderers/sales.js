// ============================================================
// js/renderers/sales.js — نسخة محسّنة ومصلحة
// ============================================================
...
async function renderSalesTable() {
  ...
  if (!tbody) {
    console.error('❌ sales-tbody غير موجود');
    return;
  }
  ...
  console.log('📊 renderSalesTable - المخزون:', inv.length, 'المبيعات:', sales.length);

  // نجيب كل الـ batch IDs من المبيعات
  ...
  // لو فيه دفعات في المبيعات مش في المخزون، نبنيها
  ...
  console.log('📦 الدفعات للعرض:', displayBatches.length);
        ...
        <tr style="background:#f0faf5"><td colspan="9" style="padding:0">
          <div style="padding:10px 14px">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:.79rem;margin-bottom:8px">
              <thead>
                <tr style="background:#e8f8ee">                  <th style="padding:4px 6px">عدد</th>
  ...
  _updateDayTotal(sales);
  console.log('✅ تم رسم الجدول بنجاح');
}

function goToProduct(batchId) {
  console.log('🎯 goToProduct called with:', batchId);
  
  if (!batchId) {
    Toast.error('❌ رقم الدفعة مفقود');
    return;
  }
  ...
  if (!batch) {
    ...
    console.error('Batch not found. Available:', store.inv);
    return;
  }

  if (typeof showPage !== 'function') {
    Toast.error('❌ دالة showPage غير موجودة');
    return;  }

  _activeBatchId = batchId;
  console.log('📍 Setting _activeBatchId to:', _activeBatchId);
  
  showPage('sales', document.querySelector('[data-page="sales"]'));
  
  setTimeout(() => {
    console.log('⏰ Calling toggleBatch for:', batchId);
    if (typeof toggleBatch === 'function') {
      toggleBatch(batchId);
      Toast.success('✅ تم فتح الدفعة');
    } else {
      Toast.error('❌ دالة toggleBatch غير موجودة');
    }
  }, 300);
}

function toggleBatch(batchId) {
  console.log('🔄 toggleBatch called with:', batchId, 'current:', _activeBatchId);
  _activeBatchId = _activeBatchId === batchId ? null : batchId;
  console.log('🔀 New _activeBatchId:', _activeBatchId);
  renderSalesTable();
}

function openInlineSaleForm(batchId) {
  console.log(' openInlineSaleForm:', batchId);
  const d = document.getElementById('sf-' + batchId);
  if (!d) {
    console.error('Element not found: sf-' + batchId);
    return;
  }
  ...
  <div style="background:#f0faf5;border-radius:9px;padding:11px;margin-top:8px;border:1.5px solid #a2d9ce">
    <div style="font-weight:800;color:var(--green);margin-bottom:8px;font-size:.85rem">+ بيعة جديدة</div>    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(80px,1fr));gap:7px;margin-bottom:8px">
      <div>
    ...
    <div style="font-size:.72rem;color:var(--gray);margin-top:6px">
      متبقي: <strong style="color:var(--blue)">${N(maxQty)} ${unit}</strong>
    </div>
  </div>`;
}
...
async function confirmInlineSale(batchId, maxQty) {
  ...
  const qty = parseFloat(document.getElementById(`sf-qty-${batchId}`)?.value) || 0;
  const wt = parseFloat(document.getElementById(`sf-wt-${batchId}`)?.value) || 0;  const price = parseFloat(document.getElementById(`sf-price-${batchId}`)?.value) || 0;
  ...
  if (qty > 0 && qty > maxQty) return Toast.error(`الكمية أكبر من المتبقي`);
  ...
  try {
    ...
    const custName = custId ? (store.custs||[]).find(c=>c.id===custId)?.name || 'العميل' : 'نقدي';
...
async function deleteSaleLine(saleId, batchId) {
  ...
  try {
    await API.sales.delete(saleId);    const [inventory, sales] = await Promise.all([
      API.inventory.list(),
      API.sales.list(store._state.currentDate)
    ]);
    store.set('inventory', inventory);
...
async function closeDay() {
  ...
  if (!remaining.length) {
    Toast.info('لا توجد بضاعة متبقية');
    return;
  }
  ...
  if (!newDate || !/^\d{4}-\d{2}-\d{2}$/.test(newDate.trim())) {
    if (newDate) Toast.warning('صيغة التاريخ YYYY-MM-DD');
    return;
  }

  if (!confirm(`ترحيل ${remaining.length} دفعة؟`)) return;

  try {
    await Promise.all(remaining.map(b =>      API.inventory.carryOver(b.batch_id, parseFloat(b.remaining_qty), newDate.trim())
    ));
